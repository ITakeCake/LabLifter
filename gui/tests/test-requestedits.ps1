...
[pscustomobject]@{ _note = 'test'; users = @('Blake','viewer1') } | ConvertTo-Json | Set-Content $script:WhitelistFile -Encoding UTF8
$script:WhitelistCache = $null
$wl = Import-Whitelist
ok (@($wl).Count -eq 2 -and ($wl -contains 'Blake') -and ($wl -contains 'viewer1')) "whitelist loads 2 users"
ok (Test-UserWhitelisted 'BLAKE')  "membership is case-insensitive (BLAKE ok)"
...
$r = Push-MasterConfig
ok ($r.ok -and $r.keys -eq 5) "Push-MasterConfig posts 5 keys (health_excluded joined 2026-08-19)"
...
ok (@($byKey['whitelist'].users) -contains 'Blake') "whitelist payload carries Blake"
...
$okReq = Request-FleetMark -Machine '55A-1' -State 'missing' -User 'Blake' -Note 'left the room'
ok ($okReq.Ok -and $okReq.Applied) "whitelisted request sent + applied"
ok ($script:CapturedReq.machine -eq '55A-1' -and $script:CapturedReq.state -eq 'missing' -and $script:CapturedReq.by -eq 'Blake') "request payload carries machine/state/by"
...
function script:Invoke-RestMethod {
    ...
    [pscustomobject]@{ ok = $true; requests = @(
        [pscustomobject]@{ machine='55A-2'; state='broken';  by='Blake'; ts='2026-08-18T10:00:00'; received_at='2026-08-18T10:00:01Z' },
        [pscustomobject]@{ machine='55A-3'; state='clear';   by='Blake'; ts='2026-08-18T10:00:00'; received_at='2026-08-18T10:00:02Z' }
    ) }
}
...
ok ($script:FleetMarks['55A-2|pending'].By -eq 'Blake') "merged mark carries requester as By"
